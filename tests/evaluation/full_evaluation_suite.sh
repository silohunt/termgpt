...
# Full Evaluation Suite - Complete test suite with 50+ commands
# Comprehensive evaluation across 5 main categories

set -e

EVALUATION_FILE="test_scenarios_50_commands.txt"
RESULTS_FILE="results/comprehensive_evaluation_results.md"
...
test_command() {
    ...
    llm_valid=0
    llm_validation=""
    if [[ -n "$llm_result" ]] && [[ ${#llm_result} -gt 5 ]] && [[ "$llm_result" =~ ^[a-zA-Z] ]] && [[ "$llm_result" != *"Error"* ]] && [[ "$llm_result" != *"TIMEOUT_OR_ERROR"* ]]; then
        if [[ "$llm_result" != *"Sorry"* ]] && [[ "$llm_result" != *"cannot"* ]] && [[ "$llm_result" != *"unable"* ]]; then
            if llm_validation=$(validate_command "$llm_result" "$query" 2>&1); then
                llm_valid=1
            else
    ...
    postproc_valid=0
    postproc_validation=""
    if [[ -n "$postproc_result" ]] && [[ ${#postproc_result} -gt 5 ]] && [[ "$postproc_result" =~ ^[a-zA-Z] ]] && [[ "$postproc_result" != *"Error"* ]] && [[ "$postproc_result" != *"TIMEOUT_OR_ERROR"* ]]; then
        if [[ "$postproc_result" != *"Sorry"* ]] && [[ "$postproc_result" != *"cannot"* ]] && [[ "$postproc_result" != *"unable"* ]]; then
            if postproc_validation=$(validate_command "$postproc_result" "$query" 2>&1); then
                postproc_valid=1
            else
    ...
    echo -e "  LLM: $llm_status$llm_validation  Post-proc: $postproc_status$postproc_validation$improvement"
